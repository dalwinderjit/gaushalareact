...
export default class BullProgenyDetail extends Component {
  ...
  action = async () => {
    ...
    if(this.context.selectedBull && this.context.selectedBull.id!==undefined){
      let data = await fetch(
        `${apiUrl}Bulls/GetCalvingDetailByBullId?id=${this.context.selectedBull.id}`,
        requestOptions
      )
        .then((res) => res.json())
        .then(
        ...
        this.props.bullProfile.bullSummary.setInterCalvPeriodData(data.interCalvPeriod);
  ...
  render() {
    //console.log("Bull calving render");
    ...
    return (
      <div className="row">
        <div className="col-md-12">
          <div className="title">Progeny Detail</div>
          <Table
            ref={(ref) => {
              this.table = ref;
            }}
