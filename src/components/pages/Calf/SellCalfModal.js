...
export default class SellCalfModal extends Modal {
  ...
  constructor(props) {
    ...
    //'Email']; //getlocation    
    this.sellCalfValidation = yup.object().shape({
      Id:yup.number().when([],()=>{
        if(this.state.taskType==='Edit'){
          return yup.number().typeError("Invalid Record Selected").required("Please select the Calf Sell Record again")
        }
      }),//typeError("Invalid cow Service ID"),
      AnimalId: yup
        .number()
        .typeError("Invalid Animal Selected")
        .required("Please Select the Calf"),
      Price: yup
        .number()
    ...
    this.formikProps = {
      initialValues: this.sellCalf,
      enableReinitialize: true,
      validateOnBlur: true,
      validateOnchange: true,
      validationSchema: this.sellCalfValidation,
      //setFieldValue:true,
      onSubmit: async (formValues, { setSubmitting, setFieldError,resetForm}) => {
        //console.log("Add Calf Service");
        var formData = new FormData();
        
        //if(formValues.cowID && formValues.cowID===this.context.selectedCalf.id)
        //let fields = ['Id','AnimalId','Price','BuyerSellerId','Date','SupervisorId','SupervisorName',
    ...
    //'Email'];
        ...
        this.templateManager.showLoading(`Selling Calf`);
        ...
        let data = await fetch(
          `${apiUrl}Calfs/${api_type}SellCalf/`,
          requestOptions
        )
          .then((res) => res.json())
          .then(
        ...
        if (data.data.status === "success") {
          this.props.cowProfile.cowEditForm.setSellCalf(true);
  ...
  selectDam = () => {
    ...
    //this.props.cowProfile.selectDamModal.type = "newCalf";
  ...
  selectSire = (setFieldValue) => {
    ...
    //this.props.cowProfile.selectSireModal.type = "newCalfService";
    //this.props.cowProfile.selectSireModal.newCalfService = this;
  ...
  updateSelectedCalf(){  //required 
    ...
    this.formRef.current.setFieldValue('cowID',this.context.selectedCalf.id);
    this.formRef.current.setFieldValue('cowName',`${this.context.selectedCalf.name}-${this.context.selectedCalf.tagNo}`);
  }
  render() {
                    ...
                    <Form id="cow_service_form" onSubmit={handleSubmit}>
                      <div className="row">
                        <div className="col-md-12">
                          <div className="title">Sell Calf</div>
                              ...
                              <tr>
                                <th colSpan="2">
                                  <div id="cs_table_title">
                                    Fill the Sell Calf Form
                                  </div>
                                  <span
                                    className="fa fa-save addCalfConceiveBtn"
                                  ...
                                  <span
                                    className="fa fa-redo addCalfConceiveBtn"
                            ...
                            <tr>
                              ...
                              <tr>
                                ...
                                <td>
                                  ...
                                  <input
                                    type="hidden"
                                    value={this.context.selectedCalf.id}
  ...
  setModalState=(taskType='Add')=>{
    if(taskType==='Add'){
      ...
      $('#addServiceDetailModal div.modal-header>h5.modal-title').html(`Add Sell Calf Detail`);
  ...
  getSellCalfDetail=async ()=>{
    let data = {cow_id:this.context.selectedCalf.id};
    
    if(this.context.selectedCalf.id!==null && this.context.selectedCalf.id!==undefined){
      let data1 = await axios.post(`${apiUrl}Calfs/GetSellCalfDetailByCalfId?cow_id=${data.cow_id}`);
      return await this.setSellCalfDetail(data1);
    }else{
      return false;
    }
  }
  setSellCalfDetail=(data)=>{
    console.log(data);
    if(data.data.status==='success'){
