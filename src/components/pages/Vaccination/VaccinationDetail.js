...
export default class VaccinationDetail extends Component {
  ...
  constructor(props) {
    ...
    this.medicationValidation = yup.object().shape({
      Id: yup.number().when([], () => {
        if (this.state.taskType === "Edit") {
          return yup
            ...
            .required("Please select the Animal Service again");
        }
      }),
      Date: yup
        .date()
      ...
      Vaccination: yup.number().required("Please Select the Vaccination"),
      AnimalID: yup.number().required("Please Select the Animal"),
      //AnimalNo:yup.number().required("Please Select the Animal"),,
    ...
    this.formikProps = {
      ...
      onSubmit: async (
        ...
        var data_ = {
          ...
          AnimalID: this.context.selectedAnimal.id,
        ...
        this.templateManager.showLoading(
          `${api_type}ing Animal Vaccination Detail`
        );
        const requestOptions = {
          method: "POST",
        };
        let data = await fetch(
          `${apiUrl}Vaccination/${api_type}VaccinationDetail?${queryString}`,
          requestOptions
        )
          .then((res) => res.json())
          .then(
    ...
    return (
      <div className="row">
        <div className="col-md-12">
          <div className="title">Vaccination Detail</div>
          <div className="row">
                    ...
                    <>
                      <Form id="medication-form" onSubmit={handleSubmit}>
                        <div>
                          ...
                          <FormField label="Animal No" values={values} setFieldValue={setFieldValue} id="AnimalId" name="AnimalId" as="empty"  placeholder="Animal Number" content={<><label
                          ...
                          <FormField label="Vaccination" values={values} setFieldValue={setFieldValue} id="Vaccination" name="Vaccination" as="select" options={this.context.vaccinations} isMulti={false} placeholder="Select Vaccination"/>
                          <FormField label="Disease" values={values} setFieldValue={setFieldValue} id="Disease" name="Disease" as="select" options={this.context.diseases} isMulti={false} placeholder="Select Disease"/>
                          <div className="input-group mb-3">
                            <label
                              className="input-group-text kg-label"
                              htmlFor="inputGroupSelect01"
                          ...
                          <div style={medication_button}>
                            <div style={float_right}>
                              ...
                              <button
                                ...
                                style={float_right}
                                type="button"
                                onClick={this.resetVaccinationForm}
                              >
                                Reset
                              </button>
                              &nbsp;&nbsp;
  ...
  resetVaccinationForm = () => {
    ...
    this.props.cowProfile.selectUserModal.clearDoctorSelection(); //reset doctors in usermodal
    this.updateSelectedAnimal();
  };
  selectDoctor = async () => {
    await this.props.cowProfile.selectUserModal.setMultipleSelection(
      this.state.doctorMultipleSelection
    );
    this.props.cowProfile.selectUserModal.setState({selectedDoctorIds:this.formRef.current.values.DoctorIDs})
  ...
  updateSelectedAnimal(){  //required 
    ...
    this.formRef.current.setFieldValue('AnimalID',this.context.selectedAnimal.id);
    this.formRef.current.setFieldValue('AnimalNo',`${this.context.selectedAnimal.tagNo}`);
    this.formRef.current.setFieldValue('AnimalName',`${this.context.selectedAnimal.name}`);
  }
  action = async (length,start,page) => {
    ...
    let data = await fetch(
      `${apiUrl}Vaccination/GetVaccinationDetailByAnimalId?id=${this.context.selectedAnimal.id}&start=${page}&length=${length}`,
      requestOptions
    )
      .then((res) => res.json())
      .then(
  ...
  actionLinks =  (row) => {
    return (
      <span
        ...
        onClick={async() => {
          console.log("Edit Vaccination",row);
          //this.props.cowProfile.cowServiceDetail.getAnimalServiceDataById(row.id);
          let data = await this.getVaccinationDetailById(row.Id);
  ...
  getVaccinationDetailById= async (id)=>{
    const requestOptions = {
      method: "POST"
    };
    let data = await fetch(
      `${apiUrl}Vaccination/GetVaccinationDetailById?id=${id}`,
      requestOptions
    )
      .then((res) => res.json())
      .then(
