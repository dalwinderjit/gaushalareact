...
export default class IndividualMedicationDetail extends Component {
    ...
    this.formikProps = {
      ...
      onSubmit: async (
        ...
        this.templateManager.showLoading(
          `${api_type}ing Animal Medication Detail`
        );
  ...
  action = async (length,start,page) => {
    ...
    let data = await fetch(
      `${apiUrl}Medication/GetMedicationDetailByAnimalId?id=${this.context.selectedAnimal.id}&start=${page}&length=${length}`,
      requestOptions
    )
      .then((res) => res.json())
      .then(
