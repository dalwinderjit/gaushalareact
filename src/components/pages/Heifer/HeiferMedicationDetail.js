...
export default class HeiferMedicationDetail extends Component {
  ...
  constructor(props) {
    ...
    this.medicationValidation = yup.object().shape({
      Id: yup.number().when([], () => {
        if (this.state.taskType === "Edit") {
          return yup
            ...
            .required("Please select the Heifer Service again");
        }
      }),
      Date: yup
        .date()
      ...
      AnimalID: yup.number().required("Please Select the Heifer"),
      //AnimalNo:yup.number().required("Please Select the Heifer"),,
    ...
    this.formikProps = {
      ...
      onSubmit: async (
        ...
        this.templateManager.showLoading(
          `${api_type}ing Heifer Medication Detail`
        );
  ...
  updateSelectedHeifer(){  //required 
    ...
    this.formRef.current.setFieldValue('AnimalNo',`${this.context.selectedHeifer.tagNo}`);
    this.formRef.current.setFieldValue('AnimalName',`${this.context.selectedHeifer.name}`);
  }
  action = async (length,start,page) => {
    ...
    let data = await fetch(
      `${apiUrl}Medication/GetMedicationDetailByAnimalId?id=${this.context.selectedHeifer.id}&start=${page}&length=${length}`,
      requestOptions
    )
      .then((res) => res.json())
      .then(
    ...
    return (
      <span
        ...
        onClick={async() => {
          ...
          //this.props.cowProfile.cowServiceDetail.getHeiferServiceDataById(row.id);
